...
import { ShieldCheck, Minus, RefreshCw, Store, Filter } from "lucide-react";
    ...
    const fetchRankings = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Extract unique outlets for filter
            ...
            <div className="mx-auto max-w-7xl space-y-8">
                ...
                {loading ? (
                    ...
                    <div className="py-24 text-center">
                        ...
                        <p className="text-xl font-bold text-gray-500">No verification officers ranked yet</p>
                    ...
                    <div className="rounded-[2rem] border border-stroke dark:border-strokedark bg-white dark:bg-boxdark shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-stroke dark:border-strokedark bg-blue-50/50 dark:bg-blue-500/5 flex items-center justify-between">
                            <p className="text-sm font-black uppercase tracking-widest text-blue-600 flex items-center gap-2">
                                <ShieldCheck size={16} /> {filtered.length} Officers Ranked
                            </p>
                            {selectedOutlet !== "all" && (
                        ...
                        <div className="divide-y divide-stroke dark:divide-strokedark">
                            {filtered.map((officer, idx) => (
                                <div key={officer.id} className="flex items-center gap-6 p-6 hover:bg-gray-50 dark:hover:bg-meta-4/20 transition-colors">
                                    ...
                                    {/* Avatar */}
                                    <div className="w-12 h-12 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-600 text-xl font-black flex-shrink-0">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-1">
                                            <p className="text-base font-black text-gray-800 dark:text-white truncate">
                                                {officer.full_name}
                                                {currentUser?.id === officer.officer_id ? <span className="ml-2 text-sm text-blue-500">(You)</span> : currentUser?.outlet_id === officer.outlet_id ? <span className="ml-2 text-xs font-bold text-gray-400">(Your Team)</span> : null}
                                            </p>
                                            {officer.tier && (
                                    ...
                                    <div className="hidden md:flex items-center gap-8 flex-shrink-0">
                                        <div className="text-center">
                                            ...
                                            <p className="text-xl font-black text-blue-600">{officer.score}</p>
                                        </div>
                                        {officer.total_verifications != null && (
                                            <div className="text-center">
                                                <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Total</p>
                                                <p className="text-base font-black text-gray-800 dark:text-white">{officer.total_verifications}</p>
                                            </div>
                                        )}
                                        {officer.approved_verifications != null && (
                                            <div className="text-center">
                                                <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Completed</p>
                                                <p className="text-base font-black text-gray-800 dark:text-white">{officer.approved_verifications}</p>
                                            </div>
                                        )}
                                        {officer.rejected_verifications != null && (
                                            <div className="text-center">
                                                <p className="text-[9px] font-black uppercase tracking-widest text-red-500 mb-0.5">Rejected</p>
                                                <p className="text-base font-black text-red-500">{officer.rejected_verifications}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
