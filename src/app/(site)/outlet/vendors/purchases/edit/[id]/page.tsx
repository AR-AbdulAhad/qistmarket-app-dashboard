...
export default function EditVendorPurchasePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (purchaseId) {
                    const purRes = await fetch(`${API_BASE}/api/outlet/vendors/purchases/${purchaseId}`, { headers });
                    const purData = await purRes.json();
                    if (purData.success) {
                        const p = purData.purchase;
                        setVendorId(p.vendor_id?.toString() || "");
                        setVendorName(p.vendor_name);
                        setPurchaseDate(new Date(p.purchase_date).toISOString().split('T')[0]);
                        if (p.due_date) setDueDate(new Date(p.due_date).toISOString().split('T')[0]);
                        setNotes(p.notes || "");
                        ...
                        if (p.items && p.items.length > 0) {
                            ...
                            p.items.forEach((item: any) => {
                                const key = `${item.product_name}-${item.category || ""}-${item.color_variant || ""}-${item.unit_price}`;
                                if (grouped.has(key)) {
                                    const existing = grouped.get(key);
                                    existing.ids.push(item.id);
                                    if (item.imei_serial) existing.imei_serials.push(item.imei_serial);
                                    else existing.imei_serials.push(...Array(item.quantity).fill(""));
                                    ...
                                    grouped.set(key, {
                                        ...
                                        imei_serials: item.imei_serial ? [item.imei_serial] : Array(item.quantity).fill(""),
        ...
        try {
            const apiItems = items.flatMap(item => {
                ...
                if (hasImeis) {
                    ...
                    const records = validImeis.map((imei, idx) => ({
                        id: item.ids && item.ids[idx] ? item.ids[idx] : undefined,
                        product_name: item.product_name,
                        category: item.category,
                        color_variant: item.color_variant,
                    ...
                    return [{
                        id: item.ids && item.ids[0] ? item.ids[0] : undefined,
                        product_name: item.product_name,
                        category: item.category,
                        color_variant: item.color_variant,
                        imei_serial: "",
            ...
            const res = await fetch(`${API_BASE}/api/outlet/vendors/purchases/${purchaseId}`, {
                method: "PUT",
            ...
            if (data.success) {
                setSuccess("Vendor purchase updated successfully!");
    ...
    return (
        <div className="mx-auto max-w-7xl">
            <Breadcrumb pageName="Edit Vendor Purchase" />

            <form onSubmit={handleSubmit} className="mb-20">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-2xl font-black text-gray-800 dark:text-white flex items-center gap-2">
                            <Plus className="text-primary" /> Edit Purchase Invoice
                        </h1>
                        <p className="text-sm text-gray-400 mt-1">Update procurement batch from vendor</p>
                    </div>
