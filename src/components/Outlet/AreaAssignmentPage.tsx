...
interface Props {
    role: 'verification' | 'delivery' | 'recovery';
    title: string;
}

const AreaAssignmentPage = ({ role, title }: Props) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [offResp, addrResp, settingsResp] = await Promise.all([
                fetch(`${BACKEND_URL}/api/assignments/officers?role=${role}`, { headers: { Authorization: `Bearer ${token}` } }),
                fetch(`${BACKEND_URL}/api/address/hierarchy`, { headers: { Authorization: `Bearer ${token}` } }),
                fetch(`${BACKEND_URL}/api/outlet/auto-assignment-settings`, { headers: { Authorization: `Bearer ${token}` } })
            ]);
            ...
            if (settingsData.success) {
                setAutoAssignEnabled(settingsData.settings[role] || false);
            }
    ...
    const handleToggleAutoAssign = async () => {
        ...
        try {
            ...
            const resp = await fetch(`${BACKEND_URL}/api/outlet/auto-assignment-settings`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                },
                body: JSON.stringify({ settings: { [role]: newValue } })
            });
            const data = await resp.json();
            if (data.success) {
                setAutoAssignEnabled(newValue);
                toast.success(`Auto-assignment ${newValue ? 'enabled' : 'disabled'}`);
            } else {
    ...
    return (
        <div className="mx-auto max-w-7xl">
            <Breadcrumb pageName={`${title} Assignments`} />

            <div className="bg-white dark:bg-gray-dark rounded-xl shadow-sm overflow-hidden mb-6">
                <div className="p-6 flex justify-between items-center bg-red-50/30 dark:bg-red-900/5">
                    <div>
                        <h3 className="text-lg font-bold text-gray-800 dark:text-white">Auto Assignment</h3>
                        <p className="text-sm text-gray-500">When enabled, orders in assigned areas will be automatically assigned to the connected officer.</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <span className={`text-sm font-semibold ${autoAssignEnabled ? 'text-green-600' : 'text-gray-400'}`}>
                            {autoAssignEnabled ? 'Enabled' : 'Disabled'}
                        </span>
                        <SwitcherOne 
                            enabled={autoAssignEnabled} 
                            setEnabled={handleToggleAutoAssign}
                            disabled={toggling}
                        />
                    </div>
                </div>
            ...
            <div className="bg-white dark:bg-gray-dark rounded-xl shadow-sm overflow-hidden">
                <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex flex-col md:flex-row justify-between items-center gap-4">
                    <h2 className="text-xl font-bold">{title} Officers</h2>
                ...
                <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/50 overflow-y-auto">
                    <div className="bg-white dark:bg-gray-dark rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                        <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full text-2xl">&times;</button>
                        </div>

                        <div className="flex-1 overflow-hidden flex">
                            {/* Zones List */}
                            <div className="w-1/4 border-r border-gray-100 dark:border-gray-800 p-4 space-y-2 overflow-y-auto">
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 px-2">Zones</p>
